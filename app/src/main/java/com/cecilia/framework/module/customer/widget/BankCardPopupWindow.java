...
public class BankCardPopupWindow extends PopupWindow {
    ...
    private BankCardAdapter mChooseAddressAdapter;
        ...
        switch (v.getId()) {
            case R.id.tv_confirm:
                ...
                if (mAddressBean == null) {
                    ToastUtil.newSafelyShow("请选择银行卡！");
                    return;
                }
    ...
    public interface OnAddressConfirmListener {
        void onConfirm(BankCardBean addressBean);
    }

    public void initView(final Activity context, final List<BankCardBean> list, int height) {
        final LayoutInflater mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View mView = mInflater.inflate(R.layout.layout_bank_card_pup, null);
...
//    this.setAnimationStyle(R.style.select_anim);
        ...
        mChooseAddressAdapter = new BankCardAdapter(context, R.layout.item_choose_bank_card);
        mRecyclerView.setAdapter(mChooseAddressAdapter);
        mChooseAddressAdapter.setDataList(list);
        mChooseAddressAdapter.setOnItemListener(new OnItemClickListener() {
            @Override
        ...
        mIvText2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                BankCardActivity.launch(context);
                dismiss();
            }
        });
