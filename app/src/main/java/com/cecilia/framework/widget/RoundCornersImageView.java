...
public class RoundCornersImageView extends AppCompatImageView {
    ...
    public RoundCornersImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //取xml文件中设定的参数
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.RoundCornersImageView);
        mType = ta.getInt(R.styleable.RoundCornersImageView_type, DEFAULT_TYPE);
        mBorderColor = ta.getColor(R.styleable.RoundCornersImageView_borderColor, DEFAULT_BORDER_COLOR);
        mBorderWidth = ta.getDimensionPixelSize(R.styleable.RoundCornersImageView_borderWidth, dip2px(DEFAULT_BORDER_WIDTH));
        mRectRoundRadius = ta.getDimensionPixelSize(R.styleable.RoundCornersImageView_rectRoundRadius, dip2px(DEFAULT_RECT_ROUND_RADIUS));
        ta.recycle();
    }
